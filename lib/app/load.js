...
module.exports = function(sails) {
  ...
  return function load(configOverride, cb) {
    ...
    // If we're given objects to merge in to the Sails object via "expose", save them
    // to the _external property and delete them from the config
    if (typeof configOverride.expose == 'object') {
      sails._external = configOverride.expose;
      delete configOverride.expose;
    } else {
      sails._external = {};
    }
