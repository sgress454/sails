...
module.exports = function(sails) {
	...
	return function onConnect (socket) {
		...
		function mapRoute (messageName) {
          ...
          // Make sure subapps hook is active, and (for now) only allow
          // subapps under the top-level sails app (i.e. no nested subapps)
          if (!sails.hooks.subapps || sails !== global.sails) {return next();}

          // See if the url matches any of the mount points
          if (!sails.util.any(global.sails.hooks.subapps.mountMap, function(app, url) {
            var regex = new RegExp("^"+url+"($|\\/)");
            if (requestCtx.req.url.match(regex)) {
              requestCtx.req.originalUrl = requestCtx.req.url = requestCtx.req.url.replace(regex,'/');
              app.emit('router:request', requestCtx.req, requestCtx.res);
              return true;
            }
          })) {
