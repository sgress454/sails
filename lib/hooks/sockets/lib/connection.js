...
module.exports = function(sails) {
	...
	return function onConnect (socket) {
		...
		function mapRoute (messageName) {
          ...
          if (sails === global.sails && requestCtx.req.url.match(/^\/admin($|\/)/)) {
            requestCtx.req.url = requestCtx.req.url.replace(/^\/admin\/?/,'/');
            // requestCtx.req.originalUrl = requestCtx.req.originalUrl.replace(/^\/admin/,'/');
            global.sails.subApps.admin.emit('router:request', requestCtx.req, requestCtx.res);
          } else {
