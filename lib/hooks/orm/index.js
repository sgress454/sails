...
module.exports = function(sails) {
  ...
  var hook = {
    ...
    configure: function() {
      ...
      // Create an index of mapped connection--these are used to transform
      // a given connection name into something else, so that subapps can
      // all have unique connection names
      this.connectionsMap = _.reduce(sails.config.connections, function(memo, connection, connectionId) {
        if (connection.mappedFrom) {
          memo[connection.mappedFrom] = connectionId;
        }
        return memo;
      }, {});

      // If the default connection needs to be mapped, do it now
      if (sails.config.models && sails.config.models.connection && this.connectionsMap[sails.config.models.connection]) {
        sails.config.models.connection = this.connectionsMap[sails.config.models.connection];
    ...
    initialize: function(cb) {
      // Merge in any external adapters
      sails.adapters = _.extend(sails.adapters || {}, sails._external.adapters);
      ...
      async.auto({
        ...
        // Load any adapters for connections with "force-true"
        ...
        // `sails.config.models.*`
        normalizedModelDefs: ['_loadModules', 'forceLoadAdapters', function normalizeModelDefs(next) {
