...
module.exports = function howto_normalizeModelDefinition (sails) {
  ...
  return function normalizeModelDefinition (modelDef, modelID) {
    ...
    // If the connection needs to be mapped, do it now
    if (sails.hooks.orm.connectionsMap[newModelDef]) {
      newModelDef.connection = sails.hooks.orm.connectionsMap[newModelDef.connection];
    }
